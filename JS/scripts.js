$(document).ready(function(){
	...
	function getNowPlayingData(){
		$.getJSON(nowPlayingURL, function(nowPlayingData){
			...
			for(let i = 0; i<nowPlayingData.data.length; i++){
			...
			//	$.getJSON(thisMovieUrl, function(movieKey){
					...
					var title = nowPlayingData.data[i].attributes.title;
					var body = nowPlayingData.data[i].attributes.body.value;
					var source = nowPlayingData.data[i].attributes.field_org;
					var link = nowPlayingData.data[i].attributes.field_url;
					var changed = nowPlayingData.data[i].attributes.changed;
					let newDate = new Date(changed)
					...
					nowPlayingHTML += '<div class="col-sm-12 eachMovie">';
						nowPlayingHTML += '<button type="button" class="btnModal" data-toggle="modal" data-target="#exampleModal'+ i + '" data-whatever="@' + i + '"><h2>'+title+'</h2>';
						nowPlayingHTML += '<h3>'+body+'</h3>'; 	
						nowPlayingHTML += '<h4>Source :<a class="source-link" target="_blank" href="'+link+'">'+source+'</a><span class="float-right">'+newDate.toDateString()+'</span></h4>'; 	
					...
					$('#movieGenreLabel').html("Latest News");
						...
						searchResultsHTML += '<div class="modal fade" id="exampleModal' + i +'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">';
							searchResultsHTML += '<div class="modal-dialog" role="document">';
								searchResultsHTML += '<div class="modal-content col-sm-12 col-lg-12">';
									...
									searchResultsHTML += '<div class="col-sm-12 movieDetails">';
